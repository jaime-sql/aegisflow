...
- **AgentOutput** — one of `fire-propagation` · `evacuation` · `resource-allocation`, with `confidence`, `outputHash`, `lineage[]` and model router metadata (`openai` primary, `deepseek` backup, `fixture` when keys are missing). Live Modal/LLM failures set `degraded: true` and **cap confidence at 0.40**. Stub/fixture chips render the same **SIM** badge as RF / Edge (right rail stays exec summary → chips → dispatch). Agent `eventId`s stay `evt_aegisfire01_agent_*` (Cascade) or `evt_svwui_agent_*` (El Salvador); lineage cites the hotspot/wind IDs currently on the map. See [`docs/agents.md`](agents.md).
