...
Phrases such as “qué está pasando”, “explicame que esta pasando”, “cómo funciona esto”, “what's going on”, and “how does this work” answer from that brief in the ask language. A situation ask is exactly four short Speak-friendly beats, with a blank line between them: what is on this watch, the honesty line (`DEMO FIXTURE` / `Wind · fallback` when that is true), a Predicted one-liner, and one agent line. A how-it-works ask is at most four bullets: layers, rail, Ask, and dispatch role. They do not fall through to the off-topic refusal, and they do not return the long English refusal wall. Mic stays browser Web Speech API; this path does not add ElevenLabs speech-to-text.
...
14. Typed Spanish, keys or not: “explicame que esta pasando” and “¿qué está pasando?” return four short Spanish beats separated by blank lines (watch, honesty, Predicted, agents) — not a paragraph and not an English refusal. “como funciona esto” and “explicame como funciona” return at most four Spanish bullets (capas, panel, Ask, despacho). “what's going on” and “how does this work” stay English in the same shapes. “hey necesito que me expliques” stays Spanish. These replies are the brief itself (**SIM**), including when an LLM key is set. Mic and the [ES|EN] chips stay as they are. This path does not add ElevenLabs speech-to-text.
