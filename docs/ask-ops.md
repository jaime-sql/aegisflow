...
13. Mic language chips default to **ES** (`es-SV`, fallback `es-ES`). Switching to **EN** uses `en-US` and is remembered in localStorage. Ask `hey necesito que me expliques como funciona` and the reply is Spanish. Type `How do layers work?` and the reply stays English.
